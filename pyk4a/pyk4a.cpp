...
const int RESOLUTION_TO_DIMS[][2] = {{0,    0},
                                     {1280, 720},
                                     {1920, 1080},
                                     {2560, 1440},
                                     {2048, 1536},
...
const char *CAPSULE_JPEG_NAME = "pyk4a NVJPEGDecoder handle";
...
static void capsule_cleanup_jpeg(PyObject *capsule) {
    NvJPEGDecoder *jpeg_handle;

    jpeg_handle = (NvJPEGDecoder *) PyCapsule_GetPointer(capsule, CAPSULE_JPEG_NAME);
...
static PyObject *device_open(PyObject *self, PyObject *args) {
    ...
    if (device_handle == NULL) {
        ...
        return Py_BuildValue("INN", K4A_RESULT_FAILED, Py_None, Py_None);
    }
    ...
    if (result == K4A_RESULT_FAILED) {
        free(device_handle);
        return Py_BuildValue("INN", result, Py_None, Py_None);
    }

    PyObject *device_capsule = PyCapsule_New(device_handle, CAPSULE_DEVICE_NAME, capsule_cleanup_device);

    NvJPEGDecoder *jpegDecoder = NvJPEGDecoder::createJPEGDecoder("jpegDecoder");
    PyObject *nvjpeg_decoder_capsule = PyCapsule_New(jpegDecoder, CAPSULE_JPEG_NAME, capsule_cleanup_jpeg);

    return Py_BuildValue("INN", result, device_capsule, nvjpeg_decoder_capsule);
}
...
static PyObject *device_get_color_control_capabilities(PyObject *self, PyObject *args) {
    ...
    k4a_result_t result = k4a_device_get_color_control_capabilities(
            *device_handle, command, &supports_auto, &min_value, &max_value, &step_value, &default_value,
            &default_mode);
    _gil_restore(thread_state);
    ...
    return Py_BuildValue("I{s:I,s:N,s:i,s:i,s:i,s:i,s:I}", result, "color_control_command", command, "supports_auto",
                         supports_auto ? Py_True : Py_False, "min_value", min_value, "max_value", max_value,
                         "step_value",
...
static PyObject *device_get_imu_sample(PyObject *self, PyObject *args) {
    ...
    if (K4A_WAIT_RESULT_SUCCEEDED == result) {
        return Py_BuildValue("I{s:f,s:(fff),s:L,s:(fff),s:L}", result, "temperature", imu_sample.temperature,
                             "acc_sample",
...
k4a_result_t nv_buffer_to_numpy(void *img_buffer, PyArrayObject **img_dst, uint32_t height, uint32_t width) {
    npy_intp dims[3];

    dims[0] = height;
    dims[1] = width;
    dims[2] = 4;
    *img_dst = (PyArrayObject *) PyArray_SimpleNewFromData(3, dims, NPY_UINT8, img_buffer);
...
static PyObject *transformation_depth_image_to_color_camera(PyObject *self, PyObject *args) {
    ...
    if (K4A_RESULT_SUCCEEDED == res) {
        res = k4a_transformation_depth_image_to_color_camera(*transformation_handle, depth_image,
                                                             *depth_image_transformed);
...
static PyObject *capture_get_color_image_nvjpeg(PyObject *self, PyObject *args) {
    k4a_capture_t *capture_handle;
    NvJPEGDecoder *jpeg_handle;
    PyObject *capture_capsule;
    PyObject *jpeg_capsule;
    int thread_safe;
    ...
    PyArg_ParseTuple(args, "OOp", &capture_capsule, &jpeg_capsule, &thread_safe);
    capture_handle = (k4a_capture_t *) PyCapsule_GetPointer(capture_capsule, CAPSULE_CAPTURE_NAME);
    jpeg_handle = (NvJPEGDecoder *) PyCapsule_GetPointer(jpeg_capsule, CAPSULE_JPEG_NAME);
...
//    k4a_image_format_t format = k4a_image_get_format(*image);
        uint8_t* in_buffer = k4a_image_get_buffer(*image);
        size_t in_size = k4a_image_get_size(*image);
        NvBuffer* out_buffer;
        uint32_t pixfmt, width, height;
        fprintf(stdout, "decoding\n");
        int ret = jpeg_handle->decodeToBuffer(&out_buffer, in_buffer, in_size, &pixfmt, &width, &height);
        fprintf(stdout, "image decoded, converting to numpy array\n");
        res = nv_buffer_to_numpy(out_buffer, &np_image, height, width);
    }
...
static PyObject *calibration_color_2d_to_depth_2d(PyObject *self, PyObject *args) {
    ...
    PyArg_ParseTuple(args, "Op(ff)O!", &capsule, &thread_safe, &source_point_x, &source_point_y, &in_depth_array,
                     &PyArray_Type);
    calibration_handle = (k4a_calibration_t *) PyCapsule_GetPointer(capsule, CAPSULE_CALIBRATION_NAME);
    ...
    thread_state = _gil_release(thread_safe);
    if (K4A_RESULT_SUCCEEDED == res) {
        res = k4a_calibration_color_2d_to_depth_2d(calibration_handle, &source_point2d, depth_image, &target_point2d,
                                                   &valid);
        k4a_image_release(depth_image);
    }
    _gil_restore(thread_state);
...
static PyObject *calibration_3d_to_2d(PyObject *self, PyObject *args) {
    ...
    res = k4a_calibration_3d_to_2d(calibration_handle, &source_point3d_mm, source_camera, target_camera,
                                   &target_point2d,
                                   &valid);
    _gil_restore(thread_state);
    if (res == K4A_RESULT_FAILED) {
...
static PyMethodDef Pyk4aMethods[] = {
        ...
        {"capture_get_color_image_nvjpeg",                    capture_get_color_image_nvjpeg,             METH_VARARGS,
                                                                                                                        "Get the color image associated with the given capture using the nvjpeg library"},
